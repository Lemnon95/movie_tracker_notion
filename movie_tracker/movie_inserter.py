...
def create_payload(database_id: str, values: dict, seen: bool) -> dict:
    properties = {
        "Title": {"type": "title", "title": [{"type": "text", "text": {"content": values["Title"]}}]},
        "Tags": {"type": "multi_select", "multi_select": [{"name": values["Tags"]}]},
        "Plot": {"rich_text": [{"type": "text", "text": {"content": values["Plot"]}}]},
        "Actors": {"rich_text": [{"type": "text", "text": {"content": values["Actors"]}}]},
        "Directors": {"rich_text": [{"type": "text", "text": {"content": values["Directors"]}}]},
        "Writers": {"rich_text": [{"type": "text", "text": {"content": values["Writers"]}}]},
        ...
        "Cover": {"files": [{"type": "external", "name": "Movie Cover", "external": {"url": values["Cover"]}}]},
...
def insert_movie(token: str, database_id: str) -> tuple:
    ...
    if not seen:
        ...
        values["Last Seen"] = last_seen if last_seen and is_valid_date(last_seen) else None
        values["Tags"] = "Seen"
